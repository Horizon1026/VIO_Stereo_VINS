...
bool Vio::RunOnce() {
    ...
    if (!frontend_->RunOnce(packed_measure->left_image->image, packed_measure->right_image->image,
    ...
    std::unique_ptr<VisualPointsMeasure> visual_measure = std::make_unique<VisualPointsMeasure>(std::move(frontend_->output_data()));
...
bool Vio::CheckPackedMeasurementValidation(const PackedMeasurement *measure) {
    ...
    RETURN_FALSE_IF(measure->left_image->time_stamp_s != measure->right_image->time_stamp_s);
    return true;
}
