...
void PublishCameraData(const std::string &csv_file_path, const std::string &image_file_root, const float period_ms, const bool is_left_camera) {
    ...
    while (std::getline(file, one_line) && !one_line.empty()) {
        ...
        GrayImage image;
        {
            // This mutex is important for multi-thread loading image. Because Visualizor2D::LoadImage is a static
            // function.
...
void TestRunVio(const uint32_t max_wait_ticks) {
    ...
    vio_system.data_manager()->ShowLocalMapInWorldFrame("Vio 3d local map", 20, true);
}
...
void ConfigAllComponentsOfVio(const std::string &log_output_root_dir) {
    ...
    vio_system.options().frontend.max_feature_number = 144;
    vio_system.options().frontend.min_feature_number = 60;
    vio_system.options().frontend.select_keyframe = true;
    ...
    vio_system.options().frontend.feature_detector.grid_filter_rows = 12;
    vio_system.options().frontend.feature_detector.grid_filter_cols = 12;
    ...
    vio_system.options().backend.gravity_w = Vec3(0.0f, 0.0f, 9.8f);
    vio_system.options().backend.max_valid_feature_depth_in_meter = 30.0f;
    vio_system.options().backend.min_valid_feature_depth_in_meter = 0.05f;
    vio_system.options().backend.default_feature_depth_in_meter = 2.0f;
    vio_system.options().backend.min_parallex_angle_of_feature_for_local_ba = 3.0f;
    vio_system.options().backend.max_tolerence_time_for_estimation_in_second = 0.04f;
    vio_system.options().backend.use_multi_view_observation = true;
    vio_system.options().backend.fix_camera_extrinsics = true;
    vio_system.options().backend.max_tolerence_reprojection_error_in_normal_plane = 0.03f;
    vio_system.options().backend.enable_local_map_store_raw_images = false;
    vio_system.options().backend.enable_recording_curve_binlog = true;
    vio_system.options().backend.log_file_name = "backend.binlog";
    ...
    vio_system.options().data_manager.max_time_s_of_imu_preintegration_block = 3.0f;
